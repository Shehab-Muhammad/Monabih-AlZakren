...
public class ActionBarRtlizer {
	...
	public void rtlize(Activity activity) {
		mActivity = activity;
		final ViewGroup actionBarView = getActionBarView();

		actionBarView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
			@Override
	...
	public ViewGroup getActionBarView() {
		...
		resId = mActivity.getResources().getIdentifier("action_bar", "id", mActivity.getPackageName());
	...
	private void arrangeViewGroups() {
		...
		//
		// // find & set homeView
		// homeView = (ViewGroup) findViewByClass("HomeView",
		// homeViewContainer);
		// }
	...
	private void arrangeInRtl(ViewGroup viewGroup, boolean isInActionBarViewGroup) {
		...
		if (!isInActionBarViewGroup && childCount < 2)
			return;
		...
		for (int i = 0; i < childCount - 1; i++) {
			...
			if (child instanceof TextView) {
				// android.graphics.Typeface titleTf = android.graphics.Typeface
				// .createFromAsset(mActivity.getAssets(),
				// "fonts/DroidNaskh-Regular.ttf");
				// ((TextView) child).setTypeface(titleTf);
				// ((TextView) child).setTextSize(20);
				DisplayMetrics displaymetrics = new DisplayMetrics();
				mActivity.getWindowManager().getDefaultDisplay().getMetrics(displaymetrics);
				int max = (int) (0.75 * displaymetrics.widthPixels);
				childLeft = childRight - Math.max(max, child.getMeasuredWidth());
			} else {
				childLeft = childRight - child.getWidth();
			}
		...
		// if last child is menuView in actionBar, move it to the left otherwise
		// append to right
		int childLeft, childRight;
		if (isInActionBarViewGroup && lastChild.getClass().toString().contains("MenuView")) {
			childLeft = 0;
			childRight = childLeft + lastChild.getMeasuredWidth();
		} else {
		...
		if (Build.VERSION.SDK_INT < 11) {
			lastChild.layout(lastChildLeft, lastChild.getTop(), lastChildRight, lastChild.getBottom());
